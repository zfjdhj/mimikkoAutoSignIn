...
def mimikko():
    ...
    if energy_info_data and energy_info_data.get('body'):
        if energy_info_data['body']['Energy'] > 0:
            ...
            title_post = title_ahead + "助手" + servant_name[energy_info_data['body']['code']] + "好感度" + str(energy_info_data['body']['Favorability'])
...
try:
    ...
    if SCKEY:
        ...
        if title_post and now_time and sign_result_post and vip_roll_post and energy_reward_post:
            print("运行成功，正在推送到微信")
            post_info_a = "?text=" + title_post + "&desp=" + post_text
            #post_info_b = "?title=" + title_post + "&desp=" + post_text
            post_data_a = requests.get(sc_api + SCKEY + '.send' + post_info_a)
